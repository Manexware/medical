...
class make_medical_prescription_invoice(osv.osv_memory):
    _name="oemedical.prescription.invoice"
    ...
    # Check if the prescription is invoice exempt, and stop the invoicing process
            ...
            if pres.patient_id.id:
                invoice_data['partner_id'] = pres.patient_id.id
                res = self.pool.get('res.partner').address_get(cr, uid, [pres.patient_id.id], ['contact', 'invoice'])
                invoice_data['address_contact_id'] = res['contact']
                invoice_data['address_invoice_id'] = res['invoice']
                invoice_data['account_id'] = pres.patient_id.property_account_receivable.id
                invoice_data['fiscal_position'] = pres.patient_id.property_account_position and pres.patient_id.property_account_position.id or False
                invoice_data['payment_term'] = pres.patient_id.property_payment_term and pres.patient_id.name.name.property_payment_term.id or False

            prods_data = {}
            for pres_id in prescriptions:
                pres = pres_request_obj.browse(cr, uid, pres_id)
                logging.debug('pres.patient_id = %s; pres.prescription_line = %s', pres.patient_id, pres.prescription_line)
    ...
    # Check for empty prescription lines
                ...
                for pres_line in pres.prescription_line:
                    logging.debug('pres_line = %s; pres_line.template.name = %s; pres_line.quantity = %s', pres_line, pres_line.template.name, pres_line.quantity)

                    if prods_data.has_key(pres_line.template.name):
                        prods_data[pres_line.template.name]['quantity'] += pres_line.quantity
                    else:
                        a = pres_line.template.medicament_id.product_id.property_account_income.id
                        if not a:
                            a = pres_line.template.medicament_id.product_id.categ_id.property_account_income_categ.id

                        prods_data[pres_line.template.name] = {'product_id':pres_line.template.medicament_id.product_id.id,
                                        'name':pres_line.template.name,
                                        'quantity':pres_line.quantity,
                                        'account_id':a,
                                        'price_unit':pres_line.template.medicament_id.product_id.lst_price}
