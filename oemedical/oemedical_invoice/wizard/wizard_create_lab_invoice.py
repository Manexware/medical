...
class create_test_invoice(osv.osv_memory):
    _name='oemedical.lab.test.invoice'
    ...
    def create_lab_invoice(self, cr, uid, ids, context={}):
        ...
        if pats.count(pats[0]) == len(pats):
            ...
            if test.patient_id.partner_id.id:
                invoice_data['partner_id'] = test.patient_id.partner_id.id
                res = self.pool.get('res.partner').address_get(cr, uid, [test.patient_id.partner_id.id], ['contact', 'invoice'])
                invoice_data['address_contact_id'] = res['contact']
                invoice_data['address_invoice_id'] = res['invoice']
                invoice_data['account_id'] = test.patient_id.partner_id.property_account_receivable.id
                invoice_data['fiscal_position'] = test.patient_id.partner_id.property_account_position and test.patient_id.partner_id.property_account_position.id or False
                invoice_data['payment_term'] = test.patient_id.partner_id.property_payment_term and test.patient_id.partner_id.property_payment_term.id or False
